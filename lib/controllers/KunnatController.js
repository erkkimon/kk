KunnatController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('kunnat', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Kunnat.find(this.getCriteria(), sort);

        return this.render('kunnatIndex', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Kunnat.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Kunnat");
                Router.go('kunnatView', {_id: _id});
            });
        }
        return this.render('kunnatInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Kunnat.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Kunnat");
            });
            Router.go('kunnatView', {_id: _id});
        }
        return this.render('kunnatUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Kunnat.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Kunnat");
        });
    },
    _loadModel: function(_id) {
        return Kunnat.findOne(_id);
    },
});